...
void PacketProcessWorker::Work()
{
    ...
    while(isRunning)
    {
        if (!context.router.DequeueElementAsChunk(PipeType::ProcessInput, ID, elementList)) 
        {   
            continue;
        }
        ...
        for (auto& element : elementList)
        {
            ...
            {
                context.router.EnqueueElement(PipeType::DBInput, ID, std::move(element)); // input
            }
            else // 일반적인 패킷 처리
            {
                element.pk->SetResult(result); // 패킷 결과 처리
                context.router.EnqueueElement(PipeType::SendThis, ID, std::move(element)); // Send Pool로 전송
            }
        }
        elementList.clear();
    }
    ...
    while(isRunning)
    {
        ...
        for (auto& element : elementList)
        {
            ...
            if (element.nextStage == ElementStage::Database) // DB 작업을 요구하는 경우
            {
                ...
                context.router.EnqueueElement(PipeType::DBInput, ID, std::move(dbElement)); // input
            }
            else // 일반적인 패킷 처리
            {
                element.pk->SetResult(result); // 패킷 결과 처리
