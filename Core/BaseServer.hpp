...
protected:
    ...
    //std::unique_ptr<BasicProcessDispatcher> dispatcher;
    ...
    static constexpr uint32_t threadPoolCount = 8;
public:
    BaseServer(bool useUDP, uint16_t port): useUDP(useUDP), port(port), sock(-1), addr{},
        //process(nullptr),
        pkPool(9000, INFINITE), processPipePool(threadPoolCount * 2), 
        sendPipePool(threadPoolCount), sessionManager(),
        services(processPipePool, sendPipePool, pkPool, sessionManager),
        managers(),
        aoi(services, threadPoolCount), db(services, threadPoolCount),
    ...
    void Start()
    {
        ...
        processPool.Start("Process Pool");
